...
任何客户端进程**首次连接**时，Novara 会弹出授权提示。批准后该进程加入白名单，后续连接不再提示。
...
4. **进程白名单**——任何进程首次连接都需你批准，持久化于 `McpAllowedProcesses`。
5. **删除独立权限**——删除操作单独一个开关（`McpDeleteEnabled`）。
6. **敏感字段脱敏**——标签匹配 `密码`、`password`、`密钥`、`secret`、`token`、`key`、`api key`、`apikey`、`api_key`、`passwd` 的字段返回为 `****`，不参与搜索，并防止改名绕过脱敏。
