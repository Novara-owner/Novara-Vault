...
- **动效设计系统** —— 导航「藏光」辉光、切页过渡、弹窗景深与逐行入场：所有动画出自同一个令牌化动效层，应用动起来前后一致，不再各自为政
- **工作区** —— 横跨五类数据的虚拟分组，切换空间即重塑所有列表
- **快速捕获** —— 全局热键随时在任意应用中记下备忘、待办或便签
- **网络活动指示器** —— Novara 仅有的出网调用（API 检测）绝不为无声：标题栏会显示目标端点
- **首启导览** —— 首次启动的四页真实 UI 导览
- **深度加固** —— 五轮全代码库穷尽核验；加密状态变更全面事务化
